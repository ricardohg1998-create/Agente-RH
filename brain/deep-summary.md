# Deep Summary

<!-- QUICK-DEEP:START -->
## Resumen profundo (auto)

- brain/project-overview.md | estado: activo | resumen: Disponer de un esqueleto de repo operativo, stack-agnostico, listo para clonar y usar con... | mod: 2026-03-08
- brain/architecture.md | estado: activo | resumen: /.agent: capa operativa del agente (reglas, workflows, templates, config). | mod: 2026-03-10
- brain/decisions.md | estado: base | resumen: Registrar decisiones tecnicas relevantes en formato DEC. | mod: 2026-03-10
- brain/milestones.md | estado: base | resumen: sin novedades | mod: 2026-03-08
- brain/backlog.md | estado: base | resumen: sin novedades | mod: 2026-03-08
- brain/open-questions.md | estado: base | resumen: sin novedades | mod: 2026-03-08
- brain/technical-debt.md | estado: base | resumen: sin novedades | mod: 2026-03-10
- brain/pitfalls-and-errors.md | estado: base | resumen: sin novedades | mod: 2026-03-08
- brain/ideas.md | estado: base | resumen: sin novedades | mod: 2026-03-08
- brain/changelog.md | estado: base | resumen: sin novedades | mod: 2026-03-10
- brain/skills-available.md | estado: base | resumen: Config en .agent/config/skills-sources.json. | mod: 2026-03-10
- brain/workflows-index.md | estado: activo | resumen: Desarrollador de profundidad (desarrollador-profundidad) -> profundidad funcional y UX. | mod: 2026-03-09
- brain/user-instructions.md | estado: base | resumen: Este archivo guarda instrucciones permanentes del usuario. | mod: 2026-03-08
<!-- QUICK-DEEP:END -->
