...
Describe 'init-project.ps1' {
  It 'ejecuta init-project sin fallar y completa validaciones' {
    $workspace = New-TestWorkspace
    try {
      # First run deep-summary so that sync doesn't fail later
      Invoke-WorkspaceScript -Workspace $workspace -RelativeScript 'scripts\update-brain-deep-summary.ps1' | Out-Null

      $result = Invoke-WorkspaceScript -Workspace $workspace -RelativeScript 'scripts\init-project.ps1' -Arguments @(
        '-InitGit',
        '-CreateCatalog',
        '-ProjectName', 'Test API',
        '-ProjectVision', 'Probar meta data',
        '-FirstDeliverable', 'un test exitoso'
      )
